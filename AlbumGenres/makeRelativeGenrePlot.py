...
allowed_years = [year for year in range(1950, 2021, 1)]
...
filtered_year_totals = genre_data.loc[allowed_year_rows & allowed_genre_rows].groupby('year').sum()['albumCount']
...
reshaped_dataset = np.array([
    np.array([
        (
            (100 * filtered_genres.loc[(genre, year), 'albumCount'] / filtered_year_totals[year])
...
fig.savefig('Relative.jpg', dpi=300)
